...
Countable in one document pass, which is why this is a `kind: "document"` rule: no selector can count to two. The logic in `packages/rules/logic/head/charset-duplicate.ts` lists the `meta[charset]` elements once and reports each one past the first. The check reads the tree, never source offsets, so it reports in all three adapters.
