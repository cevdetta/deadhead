...
`rtc` was the container for a second side of ruby annotations — the extra
...
equivalents as passing across Blink, Gecko and WebKit — the rewrite is not
theoretical.

It may stop working entirely. MDN marks `rtc` Deprecated: it "may cease to
work at any time", so even the Gecko rendering it relies on is borrowed
time.
...
Fully detectable. The selector is the bare element name `rtc`, with no
