...
`rb` was the wrapper for ruby base text — the annotated characters grouped
...
look unchanged — which is why the dead markup survives every cleanup pass.
...
so nothing visibly breaks — while MDN warns the feature "may cease to work
...
Fully detectable. The selector is the bare element name `rb`, with no
