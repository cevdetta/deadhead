...
HTML 4 had two elements for shortened words: `<abbr>` for abbreviations and `<acronym>` for
...
It does nothing `<abbr>` doesn't. The Standard requires that "user agents must treat acronym
...
Fully detectable by tag name. There is no autofix, even though replacing `acronym` with `abbr`
