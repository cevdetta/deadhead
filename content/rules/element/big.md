...
`<big>Sale ends Friday</big>` makes those words bigger than the text around them, and that is
...
just the plain `HTMLElement` interface. So the reason the author made the text bigger (a
...
Fully detectable by tag name. There is no autofix: `big` still changes how text renders, so
