...
tells the browser to reload the
document, or fetch a different one, after a delay. It predates HTTP redirects being
something an author could reach for: on early hosting you could not set a response
...
a switch device, anyone reading in a second language. Each is pulled away mid-task with no
...
search engines treat it inconsistently for that reason, and it has been a staple of
cloaking and phishing kits for two decades.

The zero-delay form, `content="0;url=…"`, is **not** a 2.2.1 failure: there is no time
limit to adjust. It keeps every other problem above, so this rule reports it too.
...
If a view must poll, fetch the data and update the DOM in place, so the
reader's scroll position, focus and history stay where they were.

## Detectability

Fully detectable. The rule reports one element identified by one attribute value, with
no context that
