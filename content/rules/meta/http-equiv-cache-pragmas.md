...
Cache values in `<meta http-equiv>` are cargo from the era when authors hoped the
server would turn markup into headers. `Cache-Control` and `Pragma` never became
pragma-table keywords, and neither did `Expires`. `ETag` and `Last-Modified` stayed out
as well. No server turns markup into headers, and no browser
reads these tags: anything outside the pragma table maps to no state. HTTP caching
(RFC 9111) is driven by response header fields.
...
Each one tells the next maintainer that caching is handled here, when
...
`packages/rules/logic/meta/http-equiv-cache-pragmas.ts`. The fix removes the
