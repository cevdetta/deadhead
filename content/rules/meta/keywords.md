...
engine had very little to go on, so it asked the page to
...
The specification defines the field and documents why it is worthless. `keywords`
is a current standard metadata name in the HTML
Standard. The entry defining it carries this note:
...
The element is inert, and it still costs. It is invisible to your readers and plainly
...
crawler consumes drifts away from what the page says without warning.

## Use instead

Delete the element. Nothing replaces it, because nothing was being done.

To describe a page to a search engine or a social card, use `<title>` and
...
Fully detectable. The rule matches a single attribute value with no context that changes
the verdict.
...
bespoke per-site integrations. `--fix` is opt-in, so the op stands.
