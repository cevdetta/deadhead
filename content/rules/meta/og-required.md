...
Countable in one document pass, which is why this is a `kind: "document"` rule: no selector can say "some but not all". The logic in `packages/rules/logic/meta/og-required.ts` censuses `property` tags starting with `og:`; zero such tags stays quiet, and any missing required name lands in the finding detail on `head`. The check reads attributes, never source offsets, so it reports in all three adapters.
