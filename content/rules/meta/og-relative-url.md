...
`<meta property="og:image" content="/images/cover.jpg">` looks correct from inside the site:
the path resolves, the file exists, and the page renders. But Open Graph values aren't read
...
`og:url` is the object's identity, not just a link. Open Graph defines it as "the canonical URL
...
card only shows up when someone shares the link. How each platform handles an invalid value
isn't documented. A site that ships relative values is relying on leniency nobody promised.
...
Fully detectable. The selector picks out the four URL-typed base properties whose `content`
