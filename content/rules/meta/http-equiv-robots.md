...
fix: { op: "none" }
replacement: "Move crawling rules to a robots.txt file in the server root; for page-level indexing use <meta name=\"robots\"> — and never disallow the same URL in robots.txt."
tags: ["head", "meta", "seo"]
impacts: ["seo"]
related: ["meta/http-equiv-description"]
---

`robots` is a metadata *name* that ended up in the `http-equiv` attribute,
...
header; the http-equiv spelling reaches no crawler.

## Why avoid

This is a mistake, not a deprecation — and a harmful one, because
directives vanish silently. An author who wrote
...
keep a page out, use noindex — and a noindex only works if the page isn't
