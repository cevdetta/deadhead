...
Some security and CORS headers work only as HTTP response headers, yet they
...
every one of them maps to no state: `x-frame-options`,
...
meta — while the replacement tells the author which header to send instead.
