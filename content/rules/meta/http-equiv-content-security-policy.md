...
description: "A <meta> policy can never equal a header policy: the spec strips frame-ancestors, report-uri and sandbox from meta-delivered policies and offers no report-only mode."
pubDate: "2026-09-14"
status: "avoid"
severity: "harmful"
standardsBasis: "spec"
...
Content Security Policy is delivered two ways: the `Content-Security-Policy`
...
invisible — no console error, just a frameable page. That is a security hole the
...
Fully detectable with a selector only:
