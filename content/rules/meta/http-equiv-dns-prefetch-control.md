...
DNS prefetching resolves link domains before they are clicked, and browsers do
...
were never acted on, so the tag itself goes away — and where a behavior was
actually intended, it moves to the response header:
...
Fully detectable: the selector matches the pragma name ASCII
...
There is no autofix. The brief shaped `remove-element`, but on HTTPS Chromium
